    ...
    var body: some View {
        VStack {
            ...
            case .success(let events): if let events = events.first(where: { Calendar.current.isDate($0.key, inSameDayAs: selectedDate) })?.value {
                ...
                if dropped {
                    LazyVStack {
                        if dropped, let allDayEvents = events.filter({ $0.allDay }).isEmpty ? nil : events.filter({ $0.allDay }) {
                        ...
                        if let timedEvents = events.filter({ $0.allDay }).isEmpty ? nil : events.filter({ $0.allDay }) {
